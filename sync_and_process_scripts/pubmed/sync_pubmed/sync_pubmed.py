...
def process_folder(first_level_folder, db_path, output_folder,  ftp_host:str='ftp.ncbi.nlm.nih.gov', pubmed_dir:str='pub/pmc/oa_package', ):
    ...
    compare_ftp_files_with_manifest(sub_dir_record=all_records, output_path=output_folder)
...
def compare_ftp_files_with_manifest(sub_dir_record, output_path):
    for obj in sub_dir_record:
        ...
        manifest_path = output_path / first_level_folder / second_level_folder/f"{first_level_folder}_{second_level_folder}_manifest.parquet"
        ...
        for gzip_file in file_list:
                ...
                try:
                    article_tar, article_info = gzip_file
                    tracker = folder_manifest.query("article_id == @article_tar")
                    if article_info.get('type') == 'file' and article_tar.endswith('.tar.gz'):
                        # Change to ftp_article_last_modified
                        article_last_modified = convert_ftp_file_modify_to_datetime(article_info)

                        # If FTP file has been updated, write new binary to storage and update manifest
                        if not tracker.empty and article_last_modified > tracker.iloc[0]['last_updated']:
                            logging.info(f'file: {article_tar} needs updating')
                            file_contents = write_gzip_data_to_storage(
                            file_name=article_tar,
                        ...
                        # else file name not in manifest, write to storage as well since it is a new file not currently tracked
                        elif tracker.empty:
                            new_row = pd.DataFrame([{'article_name': article_tar,
                                                     'last_updated':article_last_modified, 'xml_extraction_needs_update':True,
                                                        'pdf_extraction_needs_update':True, 'minerU_extraction_needs_update':True}])
                            # concatenate and reset the index if you want a clean 0…n-1 index
                            folder_manifest = pd.concat([folder_manifest, new_row], ignore_index=True)
                            write_gzip_data_to_storage(
                            file_name=article_tar,
                            # ftp_client=ftp_client,
                            first_level_folder=first_level_folder,  # Hard-coded for demonstration
                            second_level_folder=second_level_folder,
        ...
        # Python indents, once every file has been checked over, then write new manifest to disk
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="""Full download of latest oa_subset from pubmed FTP server """)

    parser.add_argument("-o", "--output_dir", help="Output base directory of downloaded files")
    parser.add_argument("--ftp_host", help="FTP host endpoint" )
    parser.add_argument("--starting_pubmed_dir", help="Starting directory for openAccess subset")
    ...
    files = []
    first_level_folders = ftp_client.nlst()
    ...
    logger.info(f"Time elapsed : {end - start}, ({len(all_records)} L2 records)")
